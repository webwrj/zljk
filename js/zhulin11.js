...
function nav(){
	...
	function oWeiX(){
		...
		oWx.onmouseover=function(){
			...
			move(oWei,{'width':288,'right':70},{'time':800});
		};
		oWx.onmouseout=function(){
			//oWei.style.right=-290+'px';
			move(oWei,{'width':0,'right':-70},{'time':800})
		};
	};
    ...
    function topBox(){
		...
		for(var i=0;i<oNav_btn.length;i++){
			(function(index){
				oNav_btn[i].onclick=function(){
					...
					var h=oNav_box[index].offsetTop-42;
					//alert(h)
		...
		addEvent(window,'scroll',function(){
			if(tUp){			
			  var ScrTop=(document.body.scrollTop||document.documentElement.scrollTop)+42;
